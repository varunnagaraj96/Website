...
if [ -n "$BUNDLE_JUTSU" ]; then
  ...
  # Fallback for a manual project-scoped copy (not installed via the plugin
  # marketplace): cast/paint and _jutsu were copied as siblings directly under
  # .claude/skills/ instead of skills/cast + skills/_jutsu inside a plugin root.
  if [ -z "$SKILL_BASE" ] || [ ! -d "$SKILL_BASE" ]; then
    if [ -n "${CLAUDE_PROJECT_DIR:-}" ] && [ -d "${CLAUDE_PROJECT_DIR}/.claude/skills/_jutsu" ]; then
      SKILL_BASE="${CLAUDE_PROJECT_DIR}/.claude/skills/_jutsu"
    fi
  fi
fi
